...
class FluidMachinery:
    def __init__(self, name, category, control_param='Stroke'):
...
class Pump(FluidMachinery):
    ...
    def fit_head_and_power_surface(self, flowrate, stroke, head, power, cv_splits=5):
        """
    ...
    def find_top_efficiency_points(self, model_head, model_eff, model_power=None, top_n=100):
        ...
        """
        intersection_points = self.find_and_plot_intersections_2d_and_3d(use_multiprocessing=True)
    ...
    def find_and_plot_intersections_2d_and_3d(self, num_points=100, use_multiprocessing=True):
        ...
        for stroke in target_strokes:
            ...
            intersections = self._compute_intersection_for_stroke(args)
        ...
        if use_multiprocessing:
            pool = Pool(10)
            results = pool.map(self._compute_intersection_for_stroke, args_list)
            pool.close()
            pool.join()
        else:
            results = [self._compute_intersection_for_stroke(args) for args in args_list]
        ...
        # 2. 시각화
        # 2-1. 2D Flowrate와 Head 시각화
        plt.figure(figsize=(10, 6))
        for stroke in target_strokes:
            intersections = [pt for pt in all_intersections if pt[1] == stroke]
            ...
            plt.scatter([pt[0] for pt in intersections], [pt[2] for pt in intersections], color='red', s=40)

        plt.xlabel('Flowrate')
        plt.ylabel('Head')
        plt.title('Flowrate vs Head with Intersections')
        plt.legend()
        plt.grid(True)
        plt.show()

        # 2-2. 3D GPR 곡면과 시스템 곡선 시각화 (stroke와의 교점 포함)
        fig = plt.figure(figsize=(12, 8))
        ax = fig.add_subplot(111, projection='3d')
        ax.plot_surface(F, S, H_gpr, alpha=0.6, cmap='viridis')
        ax.plot_surface(F, S, H_sys, alpha=0.3, color='gray')
        ax.scatter([pt[0] for pt in all_intersections], [pt[1] for pt in all_intersections],
                   [pt[2] for pt in all_intersections],
                   color='red', s=50)
        ...
        # 2-3. 3D GPR 곡면과 시스템 곡선 시각화 (효율 상위 100개와의 교점 포함)
...
def create_machine(category, name):
    machines = [Blower, Pump, Compressor]
    return machines[category](name, category)


def main():
    """
    Pump 모델 생성 및 학습 → 곡면 시각화 → 효율 최적점 탐색
    """
    pump = create_machine(1, "pump")
